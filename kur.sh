#!/usr/bin/env bash
# kur.sh — e-Devlet E-İmza'yı Apple Silicon Mac'te tek komutla derleyip kuran yardımcı betik.
#
...
REPO_URL="https://github.com/saidsurucu/edevlet-eimza-mac-arm64.git"
CLONE_DIR="$HOME/edevlet-eimza-mac-arm64"
...
if pgrep -f "$APP_NAME/Contents/MacOS" >/dev/null 2>&1; then
	...
	osascript -e 'tell application "E-Devlet E-İmza" to quit' >/dev/null 2>&1 || true
	sleep 2
fi
if [ -e "$DEST_APP" ]; then
...
# ----- Bitti -----
printf '\n'
ok "${BOLD}BİTTİ.${RST} e-Devlet E-İmza artık Launchpad ve Applications'ta."
